...
| **Asset Type** | Fraud Case |
| **Control Record** | Fraud Case Assessment |
...
| POST | `/v1/fraud-case-assessment/initiate` | Initiate |
| GET | `/v1/fraud-case-assessment` | Retrieve (list) |
| GET | `/v1/fraud-case-assessment/{crId}/retrieve` | Retrieve |
| PUT | `/v1/fraud-case-assessment/{crId}/update` | Update |
| PUT | `/v1/fraud-case-assessment/{crId}/control` | Control — body `{"action": "suspend"\|"resume"\|"terminate"}` |
...
curl -X POST localhost:8080/v1/fraud-case-assessment/initiate -H 'content-type: application/json' -d '{"note":"hello"}'
```
...
```bash
mvn -B verify
docker build -t bian/sd-fraud-diagnosis:0.1.0 .
```

## Deploy (Helm → K8s with Cilium mesh)

```bash
helm upgrade --install sd-fraud-diagnosis ./helm -n bian-risk-compliance
```

Exposed through the platform Gateway at path prefix `/sd-fraud-diagnosis` (Cilium Gateway API). Mesh policy (`CiliumNetworkPolicy`) allows: gateway ingress, same-area peers, Prometheus — everything else denied.
